...
def execute(filters=None):
	...
	for emp in sorted(att_map):
		...
		row += [filters["total_days_in_month"],total_p, total_l, total_a]
...
def get_columns(filters):
	...
	columns += [_("Days in Month") + ":Float:80",_("Total Present") + ":Float:80", _("Total Leaves") + ":Float:80",  _("Total Absent") + ":Float:80"]
	return columns
